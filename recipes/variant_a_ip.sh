#!/bin/bash
# variant_a_ip.sh - TP=4 across fabric IPs ONLY (no hostnames anywhere).
...
sleep 3
run_node local 0
echo "$(date +%FT%T) VARIANT-A-IP LAUNCHED all ranks"
# launch verification (pre-push-compile doctrine: destructive ops get proof)
sleep 20
